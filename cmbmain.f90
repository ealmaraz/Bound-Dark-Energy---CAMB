    ...
    !erick13.01.17. We remove this parameter from this file and move it to modules.f90
    !real(dl) :: fixq = 0._dl !Debug output of one q

    !erick14.01.18. We introduce these parameters for computing the growth factors
    real(dl) aiD
    real(dl) dmiD
    ...
    !erick - comment. Here we determine the initial (conformal) time
    ...
    !erick - comment. Here we can display the mode that is running and initial tau
    !write(*,1001) 'i:', q_ix, 'k:', EV%q, 'tauini:', taustart
    !1001 FORMAT ('',A2,I3,T7,A2,E15.8,T25,A7,E15.8)
    ...
    !erick - comment. Here we initialize TimeSteps. We use this information to compute transfers.
    !                 (see CalcScalarSources)
    ...
    !erick14.01.18 System of equations for the growth factors. The original system of equations is (see Djeong's thesis):
    !      D1'' + H_conf*D1' - 3/2*H_conf**2*Omega_m*D1 = 0
    !      D2'' + H_conf*D2' - 3/2*H_conf**2*Omega_m*(D2-D1**2) = 0
    !We decompose this system of equations into:
    !      dD1/dtau    = beta/a
    !      dbeta/dtau  = a*A*D1
    !      dD2/dtau    = alpha/a
    !      dalpha/dtau = a*A*(D2-D1**2)
    !where  beta = a*D1' , alpha = a*D2' , a*A = 8*pi*G*rho_{m0}/2. The growth functions are:
    !      f1 = D1'/(H_conf*D1)  &  f2 = D2'/(H_conf*D2)
    !In the code, we denote:
    !      D1 -> y(1)         beta  -> y(2)
    !      D2 -> y(3)         alpha -> y(4)
    !      a  -> aiD          a*A   -> aA
    subroutine EvolveD(dum,num,tau,y,yprime)
    use constants
    implicit none
    real dum
    integer num
    ...
    yprime(2)=aA*y(1)
    yprime(3)=y(4)/aiD
    yprime(4)=aA*(y(3)-y(1)**2/dmiD)
    end subroutine EvolveD
    ...
    subroutine CalcScalarSources(EV,taustart)
      ...
      !erick11.01.17. We introduce these variables to compute and save the info
      !               of the overdensities in the newtonian gauge (when running a single mode)
      real(dl) N0, Nstart, Nend
      real(dl) deltac_conf, deltab_conf, deltam_conf, deltag_conf, deltan_conf
      !erick15.01.18 Filename with the info of the perturbations (when running a single mode)
      Type(TIniFile) :: Ini
      character(len=:), allocatable :: perturbations_filename
      character(len=:), allocatable :: growth_factors_filename
      integer len_output_root, len_perturbations_filename, len_growth_factors_filename
      character(len=20) kstr

      !erick15.01.18 The size of the interval in log(tau) when solving for a single mode
      integer, parameter :: NumK = 10000
      real(dl) DeltaTauK, Nac

      !erick14.01.18 We introduce these variables to solve the system for the growth factors
      integer, parameter ::  NumEqsD = 4
      real(dl) cD(24), wD(NumEqsD,9), yD(NumEqsD), yprimeD(NumEqsD), atolD
      real(dl) tau_ini, a_ini, z_ini, Hconf_ini, dm_ini, dmdot_ini
      real(dl) a_D(10000), tau_D(10000), Hconf_D(10000)
      integer indD
      integer idk
      real dum
      ...
      !erick14.01.18. Initialization of the arrays for solving for the growth factors.
      wD    = 0
      yD    = 0
      atolD = 1d-8
      indD  = 1

      !erick - comment. Here we initialize 'y', which contains the values given by the initial conditions
      ...
      !erick13.01.17. tauac is the conformal time at ac. We use tauac to make the match when we run all the modes.
      !               Alternatively, a useful approximation (differences up to the third decimal point) is given by
      !                       tauac = taustart + (ac-y(1))/sqrt((grhog+grhornomass+grhoDGc)/3)
      !               However, here we use the exact value of tauac, which is returned by TimeOfz in modules.f90
      tauac = TimeOfz(1._dl/ac-1._dl)
      !erick13.01.18. If TimeOfz fails we use the aforementioned approximation
      if (ieee_is_nan(tauac)) then
         tauac = taustart + (ac-y(1))/sqrt((grhog+grhornomass+grhoDGc)/3)
      end if


      !erick11.01.17. Here we compute the evolution of the perturbations for a single mode. The process is different than
      !               if we evolve all the modes because here we are interested in knowing the value of the overdensities
      !               over the time. On the other hand, in all-modes runs we are interested in the value of the overdensities
      !               only for a >= TimpeSteps%points(2) \approx 1.e-4 to compute sources and transfers (see below). The
      !               match between the overdensity of neutrinos and the scalar field at ac is done in equations_bde_v3/
      ...
      if (fixq/=0._dl) then
         ...
         !erick15.01.18 We configure the name of the file
         len_output_root = len(trim(Ini_Read_String('output_root')))
         write(kstr,'(F8.4)') fixq
         kstr=adjustl(kstr)

         len_perturbations_filename = len_output_root+15
         allocate(character(len_perturbations_filename) :: perturbations_filename)
         perturbations_filename = trim(Ini_Read_String('output_root'))//'_k_'//trim(kstr)//'.dat'

         call CreateTxtFile(perturbations_filename,1)
         do j=1,NumK
            !erick11.01.17. To make the match, we need to implement a uniform grid in N=ln(tau/tau0)
            N0 = log(CP%tau0/CP%tau0)
            Nstart = log(taustart/CP%tau0)
            Nac = log(tauac/CP%tau0)
            DeltaTauK = (N0-Nstart)/(NumK-1)
            !Nend = Nstart+(j-1)*(N0-Nstart)/(TimeSteps%npoints-1)
            Nend = Nstart+(j-1)*(N0-Nstart)/(NumK-1)
            tauend = CP%tau0*exp(Nend)
            !erick - comment. If we implement a uniform grid in tau, then we get a very irregular space gridding in 'a'
            !tauend = taustart+(j-1)*(CP%tau0-taustart)/1000

            !erick15.01.18 Here we spot tauac and its preceding (-) and suceding (+) points on the grid in N.
            !              The point '+' is larger than 'tauac', but the separation between them is smaller
            !              than DeltaTauK, that is, the spacing between ln(tau) points. Here we show a diagram
            !                        *    *    *    *    -  tauac +   *    *
            !              To make the match, we rename Nend & tauend (corresponding to '+') with Nac and tauac
            !              (corresponding to tauac). We evolve the equations from '-' to tauac with
            !              GaugeInterface_EvolveScal, then from tauac to '+' and then we proceed with the other
            !              points '*' as usual. This allows us to make exactly the match at condensation.
            !              We use tauac instead of ac because it is known with much more accuracy than ac.
            if ((fixq < 0.01) .and. (Nend > Nac) .and. (Nend-Nac < DeltaTauK)) then
               Nend = Nac
               tauend = tauac
            end if
            ...
            !erick - comment. Quantities in the synchronous gauge
            ddelta= (yprime(3)*grhoc+yprime(4)*grhob)/(grhob+grhoc)
            delta=(grhoc*y(3)+grhob*y(4))/(grhob+grhoc)
            growth= ddelta/delta/adotoa
            !erick11.01.17. We introduce the overdensities of CDM, baryons and matter in the newtonian gauge
            !               (see CAMB notes and Ma & Bertschinger's paper to see the transformation equations
            !               between these two gauges)
            deltac_conf = y(3)-3*adotoa*(3*yprime(2)-yprime(3)*EV%q)/(EV%q**3)
            deltab_conf = y(4)-3*adotoa*(3*yprime(2)-yprime(3)*EV%q)/(EV%q**3)
            deltam_conf = (grhoc*deltac_conf+grhob*deltab_conf)/(grhob+grhoc)
            deltag_conf = y(EV%g_ix)-4*adotoa*(3*yprime(2)-yprime(3)*EV%q)/(EV%q**3)
            deltan_conf = y(EV%r_ix)-4*adotoa*(3*yprime(2)-yprime(3)*EV%q)/(EV%q**3)
            !erick11.01.17. This is the information to be exported to the output files
            write (1,'(24E20.10)') tau, y(1), y(2), y(3), y(4), y(5), y(EV%g_ix), y(EV%g_ix+1), y(EV%r_ix), y(EV%r_ix+1), y(EV%r_ix+2), y(EV%r_ix+3), y(EV%w_ix), y(EV%w_ix+1), delta, growth, adotoa, yprime(2), yprime(3), deltac_conf, deltab_conf, deltam_conf, deltag_conf, deltan_conf

            !erick14.01.18 Here we fill in the arrays of a, tau and Hconf and initialize the variables used
            !              to get the initial conditions for computing the growth factors.
            a_D(j)     = y(1)
            tau_D(j)   = tau
            Hconf_D(j) = adotoa
         ...
         !erick14.01.18 Here we create the file with the solution for the growth factors. Note that
         !              we assume the same initial conditions for D1 & D2 and beta & alpha
         len_growth_factors_filename = len_output_root+30
         allocate(character(len_growth_factors_filename) :: growth_factors_filename)
         growth_factors_filename = trim(Ini_Read_String('output_root'))//'_growth_factors_k_'//trim(kstr)//'.dat'

         call CreateTxtFile(growth_factors_filename,25)
         z_ini = 1._dl/a_ini-1._dl
         yD(1) = dm_ini
         yD(2) = a_ini*dmdot_ini
         yD(3) = dm_ini
         yD(4) = a_ini*dmdot_ini
        write (25,'(8E20.10)') tau_ini, a_ini, z_ini, Hconf_ini, yD(1), yD(2), yD(3), yD(4)

        !erick14.01.18 After we introduce the initial conditions, we solve the system of differential equations
        !              and save our results
        do j=1,9999
           tau    = tau_D(j)
           tauend = tau_D(j+1)
            aiD    = a_D(j+1)
            call dverk(dum,NumEqsD,EvolveD,tau,yD,tauend,atolD,indD,cD,NumEqsD,wD)
            write (25,'(8E20.10)') tau, aiD, 1._dl/aiD-1._dl, Hconf_D(j+1), yD(1), yD(2), yD(3), yD(4)
         end do
         close(25)
    ...
    !erick13.01.17. This is executed when we solve for all the modes (ie, fixq = 0) for computing the sources
    !               and transfers. Sources are computed taking into account the information of the perturbations
    !               from a >= TimeSteps%points(2) \approx 1e-4. In the original code, this feature poses a
    !               problem since there is no way to make the match (between the overdensities of the neutrinos
    !               and the scalar field at ac) in the integral from [astart,6.9e-4]. Therefore, we first evolve
    !               perturbations from [astart,ac] and then we make the match. Up to here, tau = taustart
    tauend = tauac
    call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
    ...
    !erick - comment. After we called EvolveScal, tau = tauac
    !write(*,*) 'tau', tau, ' y(EV%w_ix):', y(EV%w_ix)
    ...
    do j=2,TimeSteps%npoints
       ...
       if (.not. DebugEvolution .and. (EV%q*tauend > max_etak_scalar .and. tauend > taurend) &
          ...
          !erick - comment. After we call EvolveScal by the first time in the loop, tau = tauend => aend=6.9e-4.
          !                 Here is where we begin to compute sources in equations_bde_v3/output
...
101       if (CP%WantTransfer.and.itf <= CP%Transfer%num_redshifts) then
             ...
             if (abs(tau-tautf(itf)) < 1.e-5_dl) then
                !erick - comment. HERE WE CALL THE SUBROUTINE WHICH FILLS IN THE ARRAYS WITH THE TRANSFERS
                call outtransf(EV,y, tau, MT%TransferData(:,EV%q_ix,itf))
                itf=itf+1
                if (j < TimeSteps%npoints) then
    ...
    do q_ix=Evolve_q%npoints+1,MT%num_q_trans
        EV%TransferOnly=.true. !in case we want to do something to speed it up original
    ...
    do i=1,CP%Transfer%num_redshifts
       !erick - comment In principle, we have to make the match here too, although these modes are not
       !                used for the Cl computation. This subroutine is executed when we evolve all the
       !                modes. In the original code we integrate from astart to a0 with y(EV%w_xi) = 0
       !                at ac (as stated in initial). In order to make the match, we would need, as in
       !                CalcScalarSources, evolve the equations from astart to ac, then make the match,
       !                and then continue evolving the equations up to a0. The problem is that (for some
       !                unexplained reason) the software can not resume the integration from ac onwards,
       !                once we make the match for some modes. Moreover, this issue depends on the
       !                values of the parameters given in params.ini, so the same mode can be evolved
       !                successfully if we modify the parameters. Since these modes are non-linear at
       !                present time (and therefore the solutions provided by CAMB need to be corrected)
       !                and since these modes are not used for computation of the Cls, it is better to
       !                leave the code in its original form. We could assess the effect of not doing the
       !                match by comparing the results of P(k;z=0) obtained by running all the modes, with the
       !                solution obtained by running with fixq\=0 at a0. If they are very similar, the
       !                match is not necessary and therefore the initial conditions of the inhomogeneities
       !                are irrelevant. In fact, Brax, Martin & Riazuelo, PRD 62, 103505 (2000) demonstrated
       !                that this is true for IPL potentials
        !call GaugeInterface_EvolveScal(EV,tau,y,tauac,atol,ind,c,w)
