    ...
    use Recombination
    use Errors
    !erick20.12.15. We need to import some additional constants from constants.f90
    use constants
    implicit none
    public
    ...
    type CAMBparams
        ...
        !erick29.01.17. We introduce the BDE parameters in CAMBparams. These parameters are initialized
        !               in inidriver (see P%<param> in lines 134-196).
        !                         Lambdac  -> transition energy (in eV)
        !                         alphaBDE -> exponent of the potential
        !                         wDGc     -> initial value of the eos
        !                         ac       -> scale factor of the BDE meson condensation
        !                         cpot     -> constant of proportionality of the BDE potential
        real(dl) Lambdac, alphaBDE, wDGc, ac, cpot

        !erick10.02.16. This is the comoving sound horizon at the drag epoch. We introduce this
        !               parameter to get the BAO ratio rs/Dv in inidriver.
        !               rsdrag is calculated in inithermo (see below)
        real(dl) rsdrag

        !erick28.02.17. Density of radiation  (photons + massless neutrinos) today in eV**4
        !               (used in equations_bde_v3, line 97)
        real(dl) rhor0
    end type CAMBparams
    ...
    !erick08.02.15. We introduce grhoDGc (in Mpc^-2) = 8*pi*G*ac**4*rho_DGc
    !               We use this parameter to describe the evolution of the Dark Group before condensation
    !               This parameter is used in cmbmain, equations_bde_v3 & modules
    real(dl) grhoDGc

   !erick28.12.15. These parameters are used in equations_bde_v3.f90 & cmbmain, once CAMBParams_Set runs
   !               norm_pot-> normalization of the potential (in Mpc^-2)
   real(dl) Lambdac, alphaBDE, wDGc, ac, norm_pot

    !erick13.01.17. We move this parameter from cmbmain for convenience
    real(dl) :: fixq         !Debug output of one q

    !erick15.01.18 Conformal time at ac. We use this parameter to perform the match between the overdensity
    !              of neutrinos and the initial condition of \delta \phi of the BDE field. This quantity is
    !              calculated in cmbmain.f90/CalcScalarSources
    real(dl) tauac
    ...
    !erick09.04.19 Initialization of BDE parameters (not to be confused with similar ones of CAMBparams type)
    wDGc = P%wDGc
    alphaBDE = P%alphaBDE
    Lambdac = P%Lambdac
    ac =  P%ac
    ...
    !erick03.02.15. After the grhos have been initialized, we compute norm_pot of
    !               equations_bde_v3/Vofphi:
    !                      V = V_std/Mpl^2=norm_pot/(phi**alphaBDE)
    !               where
    !                      norm_pot[in Mpc^-2] = cpot*(Lc/Mpl)^(2+alpha)*Lc^2*eV2toMpcm2
    norm_pot = CP%cpot*(CP%Lambdac/sqrt(Mpl2))**(2._dl+CP%alphaBDE)*CP%Lambdac**2*eV2toMpcm2
    !erick08.02.15. Calculation of grhoDGc
    grhoDGc = c8piG_eVm2*CP%ac**4*2._dl*CP%cpot*CP%Lambdac**4/(1._dl-CP%wDGc)*eV2toMpcm2


    !erick07.12.16. adotrad is only used in inithermo to set the initial value of \dot{a}
    !               in the radiation era when the Dark Group is present. After that, \dot{a}
    !               is updated by calling equations_bde_v3/dtauda, which automatically
    !               takes into account BDE condensation. We therefore need to modify adotrad
    !               to take into account the contribution of the Dark Group
    !adotrad = sqrt((grhog+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates)))/3)
    adotrad = sqrt((grhog+grhornomass+grhoDGc+sum(grhormass(1:CP%Nu_mass_eigenstates)))/3)
    ...
    if (present(error)) then
        ...
        !erick29.12.15. We add the BDE parameters and H0
        ...
        write(*,'("Lambda_c             = ",f9.6)') CP%Lambdac
        write(*,'("10^6 a_c             = ",f9.6)') 1.E+06*CP%ac
        write(*,'("c_pot                = ",f9.6)') CP%cpot
        write(*,'("H_0                  = ",f9.6)') P%H0
        write(*,'("Om_m (1-Om_K-Om_L)   = ",f9.6)') 1-CP%omegak-CP%omegav
        write(*,'("Om_DE^0              = ",f9.6)') CP%omegav
        write(*,'("100 theta (CosmoMC)  = ",f9.6)') 100*CosmomcTheta()
        if (CP%Num_Nu_Massive > 0) then
            write(*,'("N_eff (total)        = ",f13.6)') nu_massless_degeneracy + &
    ...
    external dtauda
    !erick - comment: 3.0d4 means 3*1e4 = 30000
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Transfer - begins !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !!!!!!!!!! erick - comment. THESE ARE THE POINTS OF THE MATTER POWER SPECTRUM !!!!!!!!!!
    do il = 1, npoints
    ...
    !erick - comment. All the calculations are performed in Transfer_Get_SigmaR above
    ...
    !erick11.04.19. Here we define the format of the name of the output files for sigma8 & fsigma8
    character(len=:), allocatable :: sigma8_filename, fsigma8_filename
    integer len_fsigma8_filename, len_sigma8_filename

    len_sigma8_filename = len(trim(Ini_Read_String('output_root'))) + 11
    len_fsigma8_filename = len(trim(Ini_Read_String('output_root'))) + 12

    allocate(character(len_sigma8_filename) :: sigma8_filename)
    sigma8_filename = trim(Ini_Read_String('output_root'))//'_sigma8.dat'

    allocate(character(len_fsigma8_filename) :: fsigma8_filename)
    fsigma8_filename = trim(Ini_Read_String('output_root'))//'_fsigma8.dat'

    !erick14.01.18 We store the results for sigma8 and fsigma8 in two different files
    call CreateTxtFile(sigma8_filename,102)
    call CreateTxtFile(fsigma8_filename,103)
    ...
    do in=1, CP%InitPower%nn
        ...
        do j_PK=1, CP%Transfer%PK_num_redshifts
            ...
            !erick14.01.18. We export the results to the output files
            write(102,'(2E20.10)') CP%Transfer%redshifts(j), MTrans%sigma_8(j_PK,in)
        end do
        if (get_growth_sigma8) then
            ...
            !erick14.01.18. We export the results to the output files
                write(103,'(2E20.10)') CP%Transfer%redshifts(j), MTrans%sigma2_vdelta_8(j_PK,in)/MTrans%sigma_8(j_PK,in)
            end do
        end if
    end do
    ...
    !erick - comment. Step size in lntau
    ...
    !erick - comment. As we said before, adotrad is only used to set the initial conditions
    !                 for \dot{a} and a. In the do loop below, adot0 is updated by calling
    !                 equations_bde_f3/dtauda, which takes into account BDE condensation. Once
    !                 dta0 is updated, 'a' is properly computed.
    tau01=tauminn
    adot0=adotrad
    a0=adotrad*tauminn
    a02=a0*a0
    ...
    do i=2,nthermo
        ...
        if (matter_verydom_tau ==0 .and. a > a_verydom) then
           !erick - comment. Given the value of a_verydom, this branch is executed
           !                 only once in the matter domination era
    ...
    if (CP%DerivedParameters) then
        ...
        !erick10.02.2016. Here we initialize the value of CP%rsdrag. We export this value to P%rsdrag
        !                 in camb.f90/CAMB_GetResults in the sentence
        !                                             Params = CP
        !                just before the 'end if' of each branch
        CP%rsdrag = rs
