!!!!! BOUND DARK ENERGY QUINTESSENCE MODEL
! Modifications are indicated as follows: erickdd.mm.yy + explanatory comments
! General features:
! * The field and the potential are normalized as
!           phi = phi_std/Mpl &&&  V = V_std/Mpl^2;
!   where phi_std is the physical field and phi is the normalized field
! * The background evolution of the field is described by three dynamical variables x, y & l defined as
!     x=sqrt(8*pi*G/6)*phidot_std/Hconf  ,  y=a*sqrt(8*pi*G/3)*sqrt(V_std)/Hconf  ,  l=-V_std'/(sqrt(8*pi*G)*V_std)
! * In our model, density perturbations of the gauge group fluid before condensation is the same as
!   neutrinos. After condensation, the resulting BDE scalar field has inhomogeneities.
! * In order to solve the perturbed equations we need to modify the next routines: output and derivs (derivsv, derivst)
!!!!!
    ...
    real(dl) :: w_lam = -1_dl         !p/rho for dark energy (assumed constant)
    ...
    module Quint
      ...
      integer, parameter :: NumPoints = 1000, NumPointsEx=NumPoints+2

     !erick07.04.19. rhos in eV**4. Lc_max and Lc_min are the max and min values of Lambdac to perform
     !               the binary search when running with use_physical = F.
     !               'quint' parameters are initialized in inidriver
     real(dl) cpot_quint, alphaBDE_quint, wDGc_quint, Lambdac_quint, ac_quint, acLc_quint
     real(dl) rhog0_quint, rhon0_quint, rhor0_quint, rhom0_quint, rhoDGc_quint, rhol_quint, rhocrit0_quint
     real(dl) Lc_max, Lc_min

     real(dl) phi_a(NumPointsEx), phidot_a(NumPointsEx)!, V_a(NumPointsEx)
     real(dl) ddphi_a(NumPointsEx),ddphidot_a(NumPointsEx)
     real(dl) Nend(NumPointsEx)

     !erick07.04.19. dN is the step size in N = ln(a/ac)
     real(dl) :: dN

     !erick09.04.19. flag for use_physical
     logical uphys

    contains


      subroutine bde_h0(P,H0)
        !erick07.04.19 This subroutine computes H0 in BDE and stores the result in P%H0
        use IniFile
        use constants
        implicit none
        type(CAMBparams), intent(in) :: P
        real(dl), intent(out) :: H0
        Type(TIniFile) :: Ini
        real(dl) omrh2, ommh2, omde0

        !This omrh2 is defined as: omrh2 = 10^10/(c^2) * Omega_rh^2, where Omega_rh^2 is the physical density of radiation
        !This ommh2 is defined as: ommh2 = 10^10/(c^2) * Omega_mh^2, where Omega_mh^2 is the physical density of matter
        omrh2 = c8piG_eVm2*P%rhor0*eV2toMpcm2/3._dl
        !erick11.04.19. We read the info of ombh2 & omch2 from the ini file
        ommh2 = 1.e10*(Ini_Read_Double('ombh2')+Ini_Read_Double('omch2'))/(c**2)

        !DE density parameter today (Lambda_c has been already initialized)
	      omde0 = bde_omega_today(Lambdac_quint)

        H0 = c/(1.e3)*sqrt((omrh2+ommh2)/(1-omde0))
      end subroutine bde_h0



      function bde_omega_today(Lc)
        !erick11.04.19. This function retrieves the density parameter of dark energy today given Lambdac
        use constants

        real(dl) bde_omega_today
	      !parameters for DLSODA
        integer neq, itol, itask, istate, iopt, lrw, liw, jt
        real(dl) y(3), atol(3), rwork(70), iwork(23)
        real(dl) t, tout, rtol
        real(dl) tc, t0
        real(dl) Lc, rhoDGc, OmegaDGc

        !input parameters of DLSODA (see odepack.f for full documentation)
        neq = 3
        itol = 2
        rtol = 1.e-4
        atol(1) = 1.e-8
        ...
        jt = 1
	      ...
	      !value of ac consistent with Lc
        !whe running use_physical = F, ac_quint is changing continuosly. Once Lambdac is found
        !ac_quint is set to its definite value (line 195 in inidriver.F90)
	      ac_quint = acLc_quint/Lc

        tc = log(ac_quint/ac_quint)
        !value of N=ln(a/ac) today. Since ac depends on Lambdac, t0 also depends on Lambdac
        t0 = log(1._dl/ac_quint)

        !initial conditions for the dynamical system
        rhoDGc = 2._dl/(1._dl-wDGc_quint)*cpot_quint*Lc**4
        OmegaDGc = rhoDGc/(rhom0_quint/ac_quint**3+rhor0_quint/ac_quint**4+rhoDGc)
        y(1) = sqrt(OmegaDGc*(1._dl+wDGc_quint)/2._dl)
        y(2) = sqrt(OmegaDGc*(1._dl-wDGc_quint)/2._dl)
        y(3) = sqrt(Mpl2)*alphaBDE_quint/Lc

        !first call to DLSODA: this is just to break the initial stiffness of the system
        t = tc
        tout = 1.e-12
        CALL DLSODA(bde_dlsoda_equations,neq,y,t,tout,itol,rtol,atol,itask,istate,iopt,rwork,lrw,iwork,liw,jac,jt)
        if (istate /= 2) write(*,*) 'DLSODA failed!'
        !second call to DLSODA: we then solve the ODES from tout to present epoch
        tout = t0
        CALL DLSODA(bde_dlsoda_equations,neq,y,t,tout,itol,rtol,atol,itask,istate,iopt,rwork,lrw,iwork,liw,jac,jt)

        !see review of Copeland, Sami & Tsujikawa
        bde_omega_today = y(1)**2+y(2)**2

      end function bde_omega_today



      subroutine bde_dlsoda_equations(neq,t,y,ydot)
        !erick11.04.19. Dynamical system of background equations. The independent variable is t = ln(a/ac).
        ...
        ws = (1._dl/3._dl)*rhor0_quint/(rhor0_quint+ac_quint*exp(t)*rhom0_quint)

        ydot(1) = -3._dl*y(1) + sqrt(3._dl/2._dl)*y(3)*y(2)**2+(3._dl/2._dl)*y(1)*(2._dl*y(1)**2+(1._dl+ws)*(1._dl-y(1)**2-y(2)**2))
        ydot(2) = -sqrt(3._dl/2._dl)*y(1)*y(2)*y(3)+(3._dl/2._dl)*y(2)*(2._dl*y(1)**2+(1._dl+ws)*(1._dl-y(1)**2-y(2)**2))
        ydot(3) = -sqrt(6._dl)*y(1)*y(3)**2/alphaBDE_quint
      end subroutine bde_dlsoda_equations



      subroutine jac(neq,t,y,ml,mu,pd,nrowpd)
        !erick11.04.19. Jacobian matrix df/dy for the dynamical system in subroutine 'bde_dlsoda_equations'.
        !               f is each one of ydot; y is x, y or l
        !               This subroutine is required by DLSODA
        integer neq, ml, mu, nrowpd
        double precision t, y, pd
        dimension y(neq), pd(nrowpd,neq)
        double precision ws

        ws = (1./3.)*rhor0_quint/(ac_quint*exp(t)*rhom0_quint+rhor0_quint)

        pd(1,1) = -3._dl+3._dl/2._dl*(6._dl*y(1)*y(1)+(1._dl+ws)*(1._dl-3._dl*y(1)*y(1)-y(2)*y(2)))
        pd(1,2) = sqrt(6._dl)*y(3)*y(2)-3._dl*y(1)*y(2)*(1._dl+ws)
        pd(1,3) = sqrt(6._dl)/2._dl*y(2)*y(2)

        pd(2,1) = -sqrt(6._dl)/2._dl*y(3)*y(2)+3._dl/2._dl*y(2)*(4._dl*y(1)-2._dl*y(1)*(1._dl+ws))
        pd(2,2) = -sqrt(6._dl)/2._dl*y(3)*y(1)+3._dl/2._dl*(2._dl*y(1)*y(1)+(1._dl+ws)*(1._dl-y(1)*y(1)-3._dl*y(2)*y(2)))
        pd(2,3) = -sqrt(6._dl)/2._dl*y(1)*y(2)

        pd(3,1) = -sqrt(6._dl)/alphaBDE_quint*y(3)*y(3)
        pd(3,2) = 0.
        pd(3,3) = -2._dl*sqrt(6._dl)/alphaBDE_quint*y(1)*y(3)

      end subroutine jac



      function bde_search_lambdac(OmegaBDE0)
        !erick11.04.19. This function returns the value of Lambdac consistent with the input value of OmegaBDE0.
        !               We start assuming an upper (Lc_max) and a lower (Lc_min) bound for Lambdac leading to an upper
        !               and a lower bound of Omega, and then we perform a binary search.
        real(dl) bde_search_lambdac
        real(dl) OmegaBDE0, OmegaBDE_min, OmegaBDE_max
        real(dl) Lc_aux, OmegaBDE_aux
        integer iter
        logical binary_success

        !upper and lower bound of initial Lambdac
        Lc_max = 50._dl; Lc_min = 23._dl
        !upper and lower bound of OmegaBDE today. It is required that these bounds enclose the input value OmegaBDE0
        OmegaBDE_max = bde_omega_today(Lc_max)
        OmegaBDE_min = bde_omega_today(Lc_min)

        if (.true.) then
           if (abs(OmegaBDE_min-OmegaBDE0) > 1d-3) then
              binary_success = .false.
              !if not, do binary search in the interval
              if (OmegaBDE_min > OmegaBDE0 .or. OmegaBDE_max < OmegaBDE0) then
                 write (*,*) 'initial trial values of Lambdac must led to Omega enclosing the input value of OmegaBDE'
                 write (*,*) 'OmegaBDE_min, OmegaBDE_max, Input = ', real(OmegaBDE_min), real(OmegaBDE_max), real(OmegaBDE0)
                 stop
              end if

              do iter=1,100
                 Lc_aux = (Lc_max+Lc_min)/2._dl
                 OmegaBDE_aux = bde_omega_today(Lc_aux)
                 if (OmegaBDE_aux < OmegaBDE0) then
                    OmegaBDE_min = OmegaBDE_aux
                    Lc_min = Lc_aux
                 else
                    OmegaBDE_max = OmegaBDE_aux
                    Lc_max = Lc_aux
                 end if
                 if (OmegaBDE_max-OmegaBDE_min < 1d-4) then
                    binary_success = .true.
                    bde_search_lambdac = (Lc_max+Lc_min)/2
                    write(*,*) 'Value of Lambdac = ', bde_search_lambdac, ' found after ', iter, 'iterations'
                    exit
                 end if
              end do !iterations
              if (.not. binary_success) stop 'Search for good Lambdac value did not converge' !this shouldn't happen
           else
              bde_search_lambdac = Lc_min
              write(*,*) 'The binary search was not performed (too good to be true) Check!'
           end if !Find initial
        end if
      end function bde_search_lambdac



      subroutine bde_dverk_equations(dum,num,N,y,yprime)
        !erick11.04.19. Same as bde_dlsoda_equations. However, this subroutine is called by dverk instead
        !               of DLSODA.
        implicit none
        real dum
        integer num
        ...
        ws = (1._dl/3._dl)*rhor0_quint/(rhor0_quint+ac_quint*exp(N)*rhom0_quint)

        yprime(1) = -3._dl*y(1) + sqrt(3._dl/2._dl)*y(3)*y(2)**2+(3._dl/2._dl)*y(1)*(2._dl*y(1)**2+(1._dl+ws)*(1._dl-y(1)**2-y(2)**2))
        yprime(2) = -sqrt(3._dl/2._dl)*y(1)*y(2)*y(3)+(3._dl/2._dl)*y(2)*(2._dl*y(1)**2+(1._dl+ws)*(1._dl-y(1)**2-y(2)**2))
        yprime(3) = -sqrt(6._dl)*y(1)*y(3)**2/alphaBDE_quint

      end subroutine bde_dverk_equations



      function Vofphi(phi,deriv)
        !erick27.01.15. Scalar potential and its first and second derivatives
        !                          Vofphi = V_std/Mpl^2 = cpot*Lambdac^(4+alpha)/(Mpl2*phi^alpha)
        !               This function is called once modules.f90/CAMBParams_Set is executed and is also called
        !               by dtauda and derivs. The value of norm_pot[in Mpc^-2] is computed in modules.
        use constants
        implicit none
        real(dl) phi,Vofphi
        integer deriv
        if (deriv==0) then
           Vofphi=norm_pot/(phi**alphaBDE)
        else if (deriv ==1) then
           Vofphi=-alphaBDE*norm_pot/(phi**(1._dl+alphaBDE))
        else if (deriv ==2) then
           Vofphi=alphaBDE*(1._dl+alphaBDE)*norm_pot/(phi**(2._dl+alphaBDE))
        else
      ...
      subroutine Quint_ValsAta(a,aphi,aphidot)
        ...
        integer ix
        real(dl) Narr
        !erick12.02.15. The interpolation is done across a uniform grid of N=ln(a/ac) instead of a.
        !               Given a, Narr is the preceding value of N, whose array index is ix. We adapt
        !               the exppressions for ix, a0, b03 and ho2o6.
        !               At this point, ac (in modules) & dN (in bde_background) have been already initialized.
        ...
        !erick12.02.15. If the interpolation formula fails, this rough approximation could be useful.
        !aphi=phi_a(ix)
        !aphidot=phidot_a(ix)
      ...
      subroutine bde_background(use_physical)
        !erick11.04.19. This subroutine solves the background equations (after ac) ONCE the value
        !               of Lambdac has been determined. Here we create the output file with the
        !               background results. The idea is to use DLSODA to break the initial stiffness
        !               of the dynamical system and then to use dverk to evolve the system up to
        !               present time.
        use IniFile
        use AMLutils
        use constants
        logical use_physical

	      real(dl) aend, xend, yend, lend, Vend, ombdeend, omrend, ommend, wbdeend
        ...
        !parameters for dverk
        integer, parameter ::  NumEqs=3
        real(dl) cnum(24), w(NumEqs,9), atol_dverk, yprime(3)
        integer ind
        real dum

        real(dl) Lc, rhoDGc, OmegaDGc, ws

        Type(TIniFile) :: Ini
        character(len=:), allocatable :: background_filename
        integer len_output_root, len_background_filename
        integer i
        ...
        dN = log(1._dl/ac_quint)/(NumPoints-1)

        rhoDGc = 2._dl/(1-wDGc_quint)*cpot_quint*Lambdac_quint**4
        OmegaDGc = rhoDGc/(rhom0_quint/ac_quint**3+rhor0_quint/ac_quint**4+rhoDGc)
        y(1) = sqrt(OmegaDGc*(1._dl+wDGc_quint)/2._dl)
        y(2) = sqrt(OmegaDGc*(1._dl-wDGc_quint)/2._dl)
        y(3) = sqrt(Mpl2)*alphaBDE_quint/Lambdac_quint
        ...
        phi_a(1) = alphaBDE_quint/lend
        Vend = Vofphi(phi_a(1),0)
        phidot_a(1) = aend*sqrt(2._dl*Vend)*xend/yend

        ombdeend = xend**2+yend**2
        wbdeend = (xend**2-yend**2)/(xend**2+yend**2)

        ws = 1._dl/3._dl*rhor0_quint/(rhor0_quint+aend*rhom0_quint)
        omrend = 3._dl*ws*(1._dl-ombdeend)
        ommend = (1._dl-3._dl*ws)*(1._dl-ombdeend)

        !we create the output file
        len_output_root = len(trim(Ini_Read_String('output_root')))

        if (use_physical) then
           len_background_filename = len_output_root+15
           allocate(character(len_background_filename) :: background_filename)
           background_filename = trim(Ini_Read_String('output_root'))//'_background.dat'
        else
           len_background_filename = len_output_root+26
           allocate(character(len_background_filename) :: background_filename)
           background_filename = trim(Ini_Read_String('output_root'))//'_background_nophysical.dat'
        end if

        !here we export the initial conditions to the output file
        call CreateTxtFile(background_filename,8)
        write(8,'(12E20.10)') aend,Nend(1),xend,yend,lend,phi_a(1),phidot_a(1),Vend,omrend,ommend,ombdeend,wbdeend
        ...
        if (istate /= 2) write(*,*) 'DLSODA failed!'
	      ...
	      !second call to DLSODA: first timestep from 1.e-12 to dN
	      tout = dN
        CALL DLSODA(bde_dlsoda_equations,neq,y,t,tout,itol,rtol,atol,itask,istate,iopt,rwork,lrw,iwork,liw,jac,jt)
        if (istate /= 2) write(*,*) 'DLSODA failed!'
        ...
        !second point
        Nend(2) = tout
        aend = ac_quint*exp(Nend(2))

        xend = y(1)
        yend = y(2)
        lend = y(3)

        phi_a(2) = alphaBDE_quint/lend
        Vend = Vofphi(phi_a(2),0)
        phidot_a(2) = aend*sqrt(2._dl*Vend)*xend/yend

        ombdeend = xend**2+yend**2
        wbdeend = (xend**2-yend**2)/(xend**2+yend**2)

        ws = 1._dl/3._dl*rhor0_quint/(aend*rhom0_quint+rhor0_quint)
        omrend = 3._dl*ws*(1._dl-ombdeend)
        ommend = (1._dl-3._dl*ws)*(1._dl-ombdeend)
        !here we export the second point to the output file
        write(8,'(12E20.10)') aend,Nend(2),xend,yend,lend,phi_a(2),phidot_a(2),Vend,omrend,ommend,ombdeend,wbdeend

	      !call to dverk: here we evolve the system from t=dN up to present time
	      atol_dverk = 1d-8
        ind = 1

        do i=3, NumPointsEx
           tout = dN*(i-1)
           call dverk(dum,NumEqs,bde_dverk_equations,Nend(2),y,tout,atol_dverk,ind,cnum,NumEqs,w)
           Nend(i) = tout
           aend = ac_quint*exp(Nend(i))

           xend = y(1)
           yend = y(2)
           lend = y(3)

           phi_a(i) = alphaBDE_quint/lend
           Vend = Vofphi(phi_a(i),0)
           phidot_a(i) = aend*sqrt(2._dl*Vend)*xend/yend
           ...
           omrend = 3._dl*ws*(1._dl-ombdeend)
           ommend = (1._dl-3._dl*ws)*(1._dl-ombdeend)
           !here we export all the information to the output file
           write(8,'(12E20.10)') aend,Nend(i),xend,yend,lend,phi_a(i),phidot_a(i),Vend,omrend,ommend,ombdeend,wbdeend
        end do
        close(8)

        !required by Quint_ValsAta
        call spline(Nend,phi_a,NumPointsEx,splZero,splZero,ddphi_a)
        call spline(Nend,phidot_a,NumPointsEx,splZero,splZero,ddphidot_a)
      end subroutine bde_background
    ...
    subroutine init_background
      ...
      !erick09.04.19 we call 'bde_background' to solve the background evolution in BDE after condensation
      call bde_background(uphys)
    ...
    function dtauda(a)
      ...
      !erick08.02.15. Here we compute the contribution of BDE: first as the Dark Group diluted
      !               as radiation, then as the scalar field (whose evolution is governed by the KG equation)
      if (quintessence) then
         if (a < ac) then
            grhoa2 = grhoa2+grhoDGc
         else
            call Quint_ValsAta(a,phi,phidot)
            !erick.27.11.14. Contribution to grhoa2 from the SF
      ...
      !erick - comment. This is the Friedmann equation
    ...
    contains
      ...
      recursive subroutine GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
        ...
        !erick - comment. The value of 'a' is updated after GaugeInterface_ScalEv is executed
        call GaugeInterface_ScalEv(EV,y,tau,tauend,tol1,ind,c,w)

        !erick15.01.18. Here we match the SF overdensity with the overdensity of neutrinos when we
        !               run a SINGLE MODE (ie, fixq /= 0). Since tau is the independent variable and
        !               it is known more precisely, we do the match using tau instead of a. However,
        !               for unexplained reasons this prodecure only works for modes k < 0.01

        if ((fixq/=0._dl) .and. (fixq < 0.01_dl) .and. (tauac == tau)) then
           write(*,*) 'exact matching with \delta_\nu at: '
           write(*,*) 'tau: ', tau, 'a: ', y(1), 'ac:', ac, 'ac-a:', ac-y(1)
           !case 1: \delta \phi /= 0 & \dot \delta \phi == 0 (not affected by the value of c_pot)
           y(EV%w_ix) = -2./(1.-wDGc)*Lambdac/(alphaBDE*sqrt(Mpl2))*y(EV%r_ix)
           !case 2: \delta \phi == 0 & \dot \delta \phi /= 0
           !y(EV%w_ix+1) = sqrt(2.*cpot/(1.-wDGc**2))*(ac*Lambdac)*(Lambdac/sqrt(Mpl2))*y(EV%r_ix)
        end if

        !erick15.01.18 For modes 0.01 <= k, we match approximately as:
        if ((fixq/=0._dl) .and. (0.01_dl <= fixq) .and. (abs(ac - y(1)) < 3.e-9_dl)) then
           write(*,*) 'approximate matching with \delta_\nu at: '
           write(*,*) 'tau: ', tau, 'a: ', y(1), 'ac:', ac, 'ac-a:', ac-y(1)
           !case 1: \delta \phi /= 0 & \dot \delta \phi == 0
           y(EV%w_ix) = -2./(1.-wDGc)*Lambdac/(alphaBDE*sqrt(Mpl2))*y(EV%r_ix)
           !case 2: \delta \phi == 0 & \dot \delta \phi /= 0
           !y(EV%w_ix+1) = sqrt(2.*cpot/(1.-wDGc**2))*(ac*Lambdac)*(Lambdac/sqrt(Mpl2))*y(EV%r_ix)
        end if
      ...
      subroutine SetupScalarArrayIndices(EV, max_num_eqns)
        ...
        !erick - comment. We need to include the 'quintessence' flag
        !Dark energy
      ...
      subroutine CopyScalarVariableArray(y,yout, EV, EVout)
        ...
        !erick17.12.2015. We need to include the 'quintessence' flag
        if (quintessence .or. w_lam /= -1 .and. w_Perturb) then
        ...
        if (CP%Num_Nu_massive == 0) then
           ...
           do nu_i = 1, CP%Nu_mass_eigenstates
              ...
              do j=1, nqmax
                 ...
                 !erick - comment. adotrad has been initialized in modules/CAMBParams_Set. See the comments.
                 !                 Anyway, this branch is not executed for massless neutrinos
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine output(EV,y, j,tau,sources)
        ...
        !erick08.02.15. Variable intended to account for the contribution of the Dark Group to grho_t
        real(dl) grhoDG_t
        !erick08.02.15. Variables intended to account for the contribution of the SF to grho_t
        ...
        !erick - comment grho<>_t = 8*pi*G*rho<>(tau)*a2
        grhob_t=grhob/a
        grhoc_t=grhoc/a
        grhor_t=grhornomass/a2
        grhog_t=grhog/a2
        grhov_t=grhov*a**(-1-3*w_lam)
        !erick08.02.15 Calculation of grhoDG_t
        grhoDG_t=grhoDGc/a2
        ...
        !erick08.02.15. Here we compute the contribution of the Dark Group/SF to grho
        if (quintessence) then
           if (a < ac) then
              grhoex_t=grhoDG_t
              grho = grho+grhoex_t
              gpres = gpres+grhoDG_t/3
           else
              call Quint_ValsAta(a,phi,phidot)
        ...
        !18.12.15. We introduce a density perturbation in the gauge group, which evolves as the density perturbation
        !               of the neutrinos. After the condensation, the SF is inhomogeneous too. Since we need to know clxr
        !               to compute dgrho & dgq before ac, we relocate this branch after clxr has been initialized.
        !               This branch is aimed to get the contribution of the DG & the SF to dgrho and dgq.
        if (quintessence) then
           if (a < ac) then
              clxq = 0
              vq = 0
              dgrho = dgrho + grhoDG_t*clxr
              dgq = dgq + grhoDG_t*qr
           else
              clxq=y(EV%w_ix)
              vq=y(EV%w_ix+1)
        ...
        !erick - comment. After getting clxg, clxr, qg, qr, pig & pir, we gather all the results to obtain
        !                 dgrho, dgq & dgpi
        ...
        !erick - comment. In principle we have to replace grhor_t*pirdot by (grhor_t+grhoDG_t)*pirdot for a<ac.
        !                 However, output is executed for late times, when the DG is not present anymore.
        !                 Therefore, we don't need to do this
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine initial(EV,y, tau)
        ...
        !erick09.01.18 om is \omega=\Omega_m*H_conf/\sqrt{\Omega_r}. See section IX of CAMB notes
        !om = (grhob+grhoc)/sqrt(3*(grhog+grhonu))            !original
        om = (grhob+grhoc)/sqrt(3*(grhog+grhonu+grhoDGc))     !modification
        omtau=om*tau

        !Rv=grhonu/(grhonu+grhog)                             !original
        Rv=(grhonu+grhoDGc)/(grhonu+grhog+grhoDGc)            !modification
        Rg = 1-Rv
        ...
        !erick - comment. When the initial conditions are set, the Dark Group is present, so
        !                 we need to take into account the proper value of adotrad. This variable
        !                 is initialized in modules/CAMBParams_Set. See the comments.
        ...
        !erick - comment. According to the CAMB notes and Ma & Bertschinger's classic paper,
        !                 these numbers are pretty small (SYNCHRONOUS GAUGE!!!!)
        ...
        if (CP%Scalar_initial_condition==initial_vector) then
           ...
           !erick - comment. This branch is executed for adiabatic initial conditions
        ...
        !erick09.06.15. We set the initial conditions for the inhomogeneities of the SF
        !erick11.01.17. We make \delta \phi = 0 temporarily, since in ac we match it with \delta_nu
        if (quintessence .or. w_lam /= -1 .and. w_Perturb) then
           !original
        ...
        !erick - comment. Initial conditions (when the modes are outside the horizon)
        !write(*,*) tau, y(1), y(2), y(3), y(4), y(5), y(EV%g_ix), y(EV%g_ix+1), y(EV%r_ix), y(EV%r_ix+1), y(EV%r_ix+2), y(EV%r_ix+3), y(EV%w_ix), y(EV%w_ix+1)
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      !erick - comment. Transfer functions are defined in CAMB as
      !                         T(k) = \delta/k**2
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine derivs(EV,n,tau,ay,ayprime)
        ...
        !erick08.02.15 Variable intended to describe the evolution of the Dark Group before condensation.
        !              It is the contribution of this group to grho_t
        real(dl) phidot, phi,grhoex_t
        real(dl) grhoDG_t
        ...
        !erick - comment. In quintessence models, we don't use grhov_t
        if (w_lam==-1._dl) then
           grhov_t=grhov*a2
        else
           grhov_t=grhov*a**(-1-3*w_lam)
        ...
        !erick17.12.15. We remove the contribution from grhov_t. grho is now complete!
        !grho = grho_matter+grhor_t+grhog_t+grhov_t
        grho = grho+grhor_t+grhog_t

        !erick - comment. Here we get \dot{a}/a
        if (CP%flat) then
           adotoa=sqrt(grho/3)
           cothxor=1._dl/tau
        else
        ...
        if (EV%no_nu_multpoles) then
           ...
           !erick02.01.18 This branch is executed only for large a's, when the DG is not present already
           z=(0.5_dl*dgrho/k + etak)/adotoa
           dz= -adotoa*z - 0.5_dl*dgrho/k
           clxr=-4*dz/k
           qr=-4._dl/3*z
        ...
        !erick18.12.15. This branch is aimed to get the contribution of the DG & the SF to dgrho and dgq. We
        !               recall that DG overdensities evolve as neutrino overdesities; after condensation, then
        !               SF is also inhomogeneous. Since we need to know clxr to compute dgrho & dgq before ac,
        !               we relocate this branch after clxr is initialized.
        if (quintessence) then
           if (a < ac) then
              clxq = 0
              vq = 0
        ...
        !erick - comment: EQUATION FOR THE SCALE FACTOR (Friedmann equation)
        ayprime(1) = adotoa*a
        ...
        if (CP%flat) then
        !erick - comment: EQUATION FOR ETA (SCALAR) POTENTIAL
           !eta*k equation
        ...
        !               EQUATIONS FOR DELTA_PHI AND \DOT{DELTA_PHI}
        if (quintessence) then
           if (a < ac) then
              ayprime(EV%w_ix)= 0
              ayprime(EV%w_ix+1) = 0
        ...
        !erick - comment. Here is where the info for the transfer functions is computed
        ...
        !  CDM equation of motion: EQUATION FOR THE CDM OVERDENSITY
        clxcdot=-k*z
        ayprime(3) = clxcdot

        !  Baryon equation of motion: EQUATION FOR THE BARYON OVERDENSITY
        clxbdot=-k*(z+vb)
        ayprime(4) = clxbdot
        ...
        if (EV%TightCoupling) then
           !  ddota/a
           !erick16.02.15. We separate the contribution of DE to gpres before and after condensation
           if (quintessence) then
              if (a < ac) then
                 gpres = gpres + (grhog_t+grhor_t+grhoDG_t)/3
              else
           ...
           if (second_order_tightcoupling) then
              ...
              !erick15.01.18 Here we account for the contribution of the Dark Group as relativistic particles.
              !              This contribution vanishes once the BDE field condenses
              if (quintessence) then
                 if (a < ac) then
                    dgs = grhog_t*pig+(grhor_t+grhoDG_t)*pir
                 else
                    dgs = grhog_t*pig+grhor_t*pir     !erick - comment: original line
                 end if
              end if
        ...
        !erick - comment: once we get vbdot, we get the EQUATION FOR THE BARYON VELOCITY
        ayprime(5)=vbdot

        if (.not. EV%no_phot_multpoles) then
           !  Photon equations of motion. EQUATIONS OF MOTION FOR THE PHOTON OVERDENSITY AND HEAT FLUX
        ...
        if (.not. EV%no_nu_multpoles) then
           !  Massless neutrino equations of motion. EQUATIONS OF MOTION FOR THE NEUTRINO OVERDENSITY AND HEAT FLUX
      ...
      subroutine derivsv(EV,n,tau,yv,yvprime)
        ...
        real(dl) pir,adotoa
        !erick08.02.15 Variables intended to describe the evolution of the Dark Group before condensation. It is the
        !              contribution of this group to grho_t
        ...
        !erick08.02.15. Here we compute the contribution of the DG/SF to grho
        if (quintessence) then
           if (a < ac) then
              grhoex_t=grhoDG_t
              grho = grho+grhoex_t
        ...
        !erick - comment. Maybe we should modify this line, but anyway in our analysis
        !                 we are not considering vector perturbations
        !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine derivst(EV,n,tau,ayt,aytprime)
        ...
        real(dl) cothxor
        !erick08.02.15 Variable intended to describe the evolution of the Dark Group before condensation. It is the
        !              contribution of this group to grho_t
